...
<div id="wrap">
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-8">
                <div>
                    <span><a href="/pan?wd={{ $wd }}">全部结果</a></span>&nbsp;&nbsp;&nbsp;
                @foreach ($tags as $tag)
                    <span><a href="/pan?wd={{ $wd . ' ' . $tag['label_with_op'] }}">{{ $tag['label'] }}</a></span>&nbsp;&nbsp;&nbsp;
                @endforeach
                </div>
                <ul class="list-group">
                ...
                {{ $items->appends(array('wd' => Input::get('wd')))->links() }}
            </div>
            <div class="col-md-4">
                自定义搜索的广告
            </div>
